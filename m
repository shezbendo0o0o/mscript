...
clear
echo -e "Enter the keyboard shortcut that will open "$TITLE" from mscript's main menu"
   ...
   then
		...
		then
			...
			then
				...
				read DOMAIN
				clear
				/usr/local/bin/mscript-sniper "$DOMAIN"
			else
				echo -e "$TNI"
				read INSTALL
				if [[ "$INSTALL" = "install" ]]
...
function update_mscript
{
	...
	then
		...
		then
			...
			then
				clear
				#~ echo -e "Making new directory for the old mscript (will pass it to /root/mscriptold)"
				sleep 1
				if [[ -d /root/mscriptold ]]
				then
					rm -r /root/mscriptold
				fi
				mkdir /root/mscriptold
				echo -e ""
				#~ echo -e "Moving mscript to mscriptold"
				sleep 1
				mv "$LPATH" /root/mscriptold
				...
				clear
				sleep 1
				if [[ -f /root/mscriptold/mscript/IAGREE.txt ]]
				then
					cp /root/mscriptold/mscript/IAGREE.txt "$LPATH"
				fi
				if [[ -d /root/mscriptold/mscript/ks ]]
				then
					echo -e "Copying your shortcuts"
					cp -r /root/mscriptold/mscript/ks "$LPATH"
					echo -e "Done."
					sleep 0.2
				fi
				if [[ -d /root/mscriptold/mscript/settings ]]
				then
					echo -e "Copying your settings"
					cp -r /root/mscriptold/mscript/settings "$LPATH"
					echo -e "Done."
					sleep 0.2
				fi
				if [[ -f /root/mscriptold/mscript/wlanmon.txt ]]
				then
					echo -e "Copying your interfaces"
					cp /root/mscriptold/mscript/wlanmon.txt "$LPATH"
					echo -e "Done."
					sleep 0.2
				fi
				echo -e ""
				if [[ -f /root/mscriptold/mscript/wlan.txt ]]
				then
					cp /root/mscriptold/mscript/wlan.txt "$LPATH"
					echo -e "Done."
					sleep 0.2
					clear
				fi
...
function tools_menu
{
	...
	clear
		...
		then
			...
			echo -e "	From here you can install any tool available in the MOU script."
...
do
	sm=0
	clear
	if [[ ! -d /bin/mscript/smtp ]]
	then
		mkdir /bin/mscript/smtp
	fi
	...
	then
		...
		read SMTPPASS
		clear
		echo "$SMTPEMAIL" > /bin/mscript/smtp/smtpemail.txt
		echo "$SMTPPASS" > /bin/mscript/smtp/smtppass.txt 
		echo -e "Credentials saved on /bin/mscript/smtp"
		sleep 3
	elif [[ "$SMTP" = "4" ]]
	then
		clear	
	...
	then
		if [[ -f /bin/mscript/smtp/smtpemail.txt ]]
		then
			rm /bin/mscript/smtp/smtpemail.txt
			echo -e "Username removed"
		else
			echo -e "Not username found"
		fi
		if [[ -f /bin/mscript/smtp/smtppass.txt ]]
		then
			rm /bin/mscript/smtp/smtppass.txt
	...
	then
		...
		read smtppass < /bin/mscript/smtp/smtppass.txt
		read smtpemail < /bin/mscript/smtp/smtpemail.txt
		clear
...
function new_terminal
{
	...
	then
		...
		then
			...
			FJC=0
			export FJC
			gnome-terminal --geometry 87x35+9999+0 -- mo1
			sleep 1
			exec bash "$0"
			break
		elif [[ "$YORNAA" = "11" ]]
		then
			...
			echo -e "Moving into new terminal..."
			sleep 1
			gnome-terminal --geometry 80x25+9999+0 -- mo3
			sleep 1
			exec bash "$0"
			break
		elif [[ "$YORNAA" = "12" ]]
		then
			echo -e "Moving into new terminal..."
			sleep 1
			gnome-terminal --geometry 80x25+9999+0 -- mo4
			sleep 1
			exec bash "$0"
			break
		fi
...
{
file2="/root/mscript/sqltemp2"
file3="/root/mscript/sqltemp3"
file4="/root/mscript/sqltemp34"
while true
do
	clear
...
function cromos_menu
{
	...
	then
		...
		echo -e ""$RS"Fix this issue by installing it within mscript"$CE""
		echo -e "$PAKTGB"
		$READAK
	else
		cd /root/cromos
...
function donate_option
{
	...
	echo -e ""$LBS"Total mscript donations:"$CE" $DONATIONS"
	...
	if [[ "$DON" = "1" ]]
	then
		gio open "https://www.paypal.com/donate/?token=XuYIR-Uah4Q6o8AXP2-AdYyxtnjgQcVsKZ7OtMmI8V09PHe4JitF2u-gasjNlgevy4hYQ0&country.x=EG&locale.x=EG"
	elif [[ "$DON" = "2" ]]
	then
		echo -e "Donation link: https://www.paypal.com/donate/?token=XuYIR-Uah4Q6o8AXP2-AdYyxtnjgQcVsKZ7OtMmI8V09PHe4JitF2u-gasjNlgevy4hYQ0&country.x=EG&locale.x=EG"
		echo -e "$PAKTGB"
		$READAK
	...
	do
		...
		then
			...
			then
				...
				echo -e "   Then select the 'set your authtoken' option in mscript, and paste it."
...
do
	...
	tempfile="/root/mscript/tempscan.txt"
...
function install_operative-framework
{
        echo -e ""$YS"Installing Operative-framework..."$CE""

        apt-get update
        apt-get install -y git python2.7 curl

        cd /opt || return

        rm -rf /opt/Operative-Framework /opt/Operative-Framework /opt/Operative-Framework

        git clone --depth 1 https://github.com/H4CK3RT3CH/Operative-Framework.git /opt/Operative-Framework

        if [[ ! -d /opt/Operative-Framework ]]
        then
                echo -e ""$RS"Failed to clone Operative-framework."$CE""
                sleep 3
                return 1
        fi

        cat > /opt/Operative-Framework/operative.py <<'PYOP'
...
PYOP
        ...
        chmod +x /opt/Operative-Framework/operative.py
        chmod -R a+rX /opt/Operative-Framework

        cat > /usr/local/bin/operative-framework <<'EOF'
#!/usr/bin/env bash
cd /opt/Operative-Framework || exit 1
exec python2.7 operative.py "$@"
EOF

        chmod +x /usr/local/bin/operative-framework
        ln -sf /usr/local/bin/operative-framework /usr/bin/operative-framework

        echo -e ""$GS"Operative-framework installed successfully."$CE""
        echo -e ""$YS"Run it with: operative-framework"$CE""
        sleep 3
}
